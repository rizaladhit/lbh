<x-app-layout>
    <x-slot name="header">Master Status Perkara</x-slot>
    ...
    <div class="panel">
        <div class="panel-head">
            <div class="d-flex align-items-center gap-3">
                ...
                <div>
                    ...
                    <div style="font-size:.72rem;color:#94a3b8;">Total {{ $statusPerkaras->count() }} status terdaftar</div>
                </div>
            </div>
            <a href="{{ route('status-perkara.create') }}" class="btn-cta">
        ...
        <div class="table-responsive">
            <table class="data-table table">
                ...
                <tbody>
                    @forelse($statusPerkaras as $sp)
                    <tr>
                        <td style="padding-left:24px;">
                            <span style="font-size:.8rem;color:#94a3b8;">{{ $loop->iteration }}</span>
                        </td>
                        <td>
                            <div style="font-size:.85rem;font-weight:600;" class="text-body">{{ $sp->nama }}</div>
                        </td>
                        <td>
                            <span class="{{ $sp->getBadgeClass() }}">{{ $sp->nama }}</span>
                        </td>
                        <td>
                            @if($sp->is_final)
                        ...
                        <td>
                            <span style="font-size:.85rem;" class="text-body">{{ $sp->urutan }}</span>
                        </td>
                        <td style="padding-right:24px;text-align:right;">
                            <div class="d-flex justify-content-end gap-1">
                                <a href="{{ route('status-perkara.edit', $sp) }}" class="action-btn" style="color:#f59e0b;" title="Edit">
                                    <i class="fa-solid fa-pen-to-square"></i>
                                </a>
                                <form action="{{ route('status-perkara.destroy', $sp) }}" method="POST" style="display:inline;"
                    ...
                    <tr>
                        <td colspan="6">
                            <div class="empty-state">
                                ...
                                <a href="{{ route('status-perkara.create') }}" class="btn-cta" style="margin:auto;width:fit-content;">
