<x-app-layout>
    ...
    @endphp
    <x-slot name="header">{{ $isEdit ? 'Edit' : 'Buat' }} Laporan Penasehat Hukum - Pengadilan Subang</x-slot>

    <div class="row justify-content-center">
        <div class="col-lg-8">
            <div class="card shadow-sm border-0">
                <div class="card-header">{{ $isEdit ? 'Edit' : 'Form' }} Laporan Pengadilan Subang</div>
                <div class="card-body p-4">
                    <form method="POST" action="{{ $isEdit ? route('laporan-ph.pengadilan.update', $report) : route('laporan-ph.pengadilan.store') }}">
                        @csrf
                        @if($isEdit)
                            @method('PUT')
                        @endif
