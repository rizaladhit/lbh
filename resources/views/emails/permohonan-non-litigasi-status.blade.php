...
<head>
    <meta charset="utf-8">
    <title>Update Status Permohonan Non-Litigasi</title>
</head>
<body style="font-family: Arial, sans-serif; color: #1f2937; line-height: 1.6; margin: 0; padding: 24px; background: #f8fafc;">
    <div style="max-width: 640px; margin: 0 auto; background: #ffffff; border: 1px solid #e5e7eb; border-radius: 8px; overflow: hidden;">
        ...
        <div style="background: {{ $headerColor }}; padding: 20px 24px;">
            <h1 style="font-size: 18px; margin: 0; color: #ffffff;">Update Permohonan Non-Litigasi</h1>
            <p style="margin: 4px 0 0; font-size: 13px; color: rgba(255,255,255,.8);">No. Registrasi: {{ $permohonanNonLitigasi->no_registrasi }}</p>
        </div>

        <div style="padding: 24px;">
            <p>Yth. {{ $permohonanNonLitigasi->nama_pemohon }},</p>
            ...
            <table style="width: 100%; border-collapse: collapse; margin: 16px 0;">
                ...
                @if($catatanNilai)
                <tr>
                    <td style="padding: 8px; border-bottom: 1px solid #e5e7eb; font-weight: bold;">{{ $catatanLabel }}</td>
                    <td style="padding: 8px; border-bottom: 1px solid #e5e7eb;">{{ $catatanNilai }}</td>
                </tr>
                @endif
                @if($permohonanNonLitigasi->assignedLawyer)
                <tr>
                    <td style="padding: 8px; border-bottom: 1px solid #e5e7eb; font-weight: bold;">Advocate</td>
                    <td style="padding: 8px; border-bottom: 1px solid #e5e7eb;">{{ $permohonanNonLitigasi->assignedLawyer->name }}</td>
                </tr>
                @endif
                ...
                <tr>
                    ...
                    <td style="padding: 8px; border-bottom: 1px solid #e5e7eb;">{{ $permohonanNonLitigasi->assignedParalegal->name }}</td>
                </tr>
                @endif
            </table>
            ...
            <p style="margin: 24px 0;">
                <a href="{{ route('permohonan-non-litigasi.show', $permohonanNonLitigasi) }}"
                   style="display: inline-block; background: {{ $headerColor }}; color: #ffffff; text-decoration: none; padding: 10px 16px; border-radius: 6px; font-weight: bold;">
                    Lihat Permohonan
                </a>
            </p>
